...
typedef struct {
    ...
    decl_fl_var   *fl_variable;     
    int fl_variable_count;

    decl_var_char *char_variable;   
    ...
    decl_fl_arr   *fl_array;        
    int fl_array_count;

    decl_char_arr *char_array;      
    ...
    decl_fl_matr   *fl_matrix;      
    int fl_matrix_count;

    decl_char_matr *char_matrix;    
...
int is_function_(const char *name_or_declaration){
    ...
    if(strstr(name_or_declaration,"__") || strchr(name_or_declaration,'(')) 
        sscanf(name_or_declaration,"__%15[^(](%64[^)]",name,args); // __name(args)
    else 
        strcpy(name,name_or_declaration);

    for(i = 0; i < return_state; i++ ){
        if(strcmp( state_stack[i].nome_function,name) == 0){
            return 1; //is function
        }
    }

    return 0; // non è una function
}
...
void *vm_alloc(size_t bytes) {
    ...
    if (vm->heap_used + aligned > vm->heap_cap) {
        printf("FATAL: memoria esaurita (%zu KB usati su %zu)\n",
               vm->heap_used/1024, vm->heap_cap/1024);
        exit(1);
    }
    void *ptr = vm->heap + vm->heap_used;
    vm->heap_used += aligned;
    return ptr;
}

void vm_create(size_t cap_kb) {
    size_t cap   = cap_kb * 1024;
    size_t total = sizeof(VM) + cap;

    vm = malloc(total);
    if (!vm) { printf("FATAL: malloc fallita\n"); exit(1); }
    memset(vm, 0, total);

    vm->heap = (char *)vm + sizeof(VM);
    vm->heap_cap  = cap;
    vm->heap_used = 0;
    ...
    vm->variable      = vm_alloc(max_number_of_var * sizeof(decl_var));
    vm->fl_variable   = vm_alloc(max_number_of_var * sizeof(decl_fl_var));
    vm->char_variable = vm_alloc(max_number_of_var * sizeof(decl_var_char));

    vm->array         = vm_alloc(max_number_of_array * sizeof(decl_arr));
    vm->fl_array      = vm_alloc(max_number_of_array * sizeof(decl_fl_arr));
    vm->char_array    = vm_alloc(max_number_of_array * sizeof(decl_char_arr));

    vm->matrix        = vm_alloc(max_number_of_array * sizeof(decl_matr));
    vm->fl_matrix     = vm_alloc(max_number_of_array * sizeof(decl_fl_matr));
    vm->char_matrix   = vm_alloc(max_number_of_array * sizeof(decl_char_matr));
...
void exec_print(char *text){
    ...
    if(n < 1){
        ...
        if(strcmp(raw,"status") == 0) { printf("%zukb",(sizeof(VM) + vm->heap_used) / 1024); return;}
...
void* exec_funarg(char *name_plus_args, int op_code, char type_return){
    
    printf("DEB: funarg chiamata con name+args: %s, op_code: %d, type_ret: %c\n",name_plus_args,op_code,type_return);
    ...
    */
    ...
    //SEARCH FUNC NAME
    if(strstr(name_plus_args,"__") || strchr(name_plus_args,'(')) 
    ...
    /*VARIABILI*/
        ...
        if (right_operand[0] == '\'') { /*var x k*/
            ...
            *dest = right_operand[1];
            ...
            sscanf(left_operand, "%63s", left_name);
            sscanf(right_operand, "__%63s", right_name);
            ...
            if (ltype == 'i') {
                int *dest = (int *)resolve(ltype, left_name);
                if (!dest) { printf("ERROR: null pointer\n"); return; }
                *dest = *(int *)exec_funarg(right_name,1,'i');
            } 
            else if (ltype == 'l') {
                float *dest = (float *)resolve(ltype, left_name);
                if (!dest) { printf("ERROR: null pointer\n"); return; }
                *dest = *(float *)exec_funarg(right_name,1,'l');
            } 
            else if (ltype == 'c') {
                char *dest = (char *)resolve(ltype, left_name);
                if (!dest) { printf("ERROR: null pointer\n"); return; }
                *dest = *(char *)exec_funarg(right_name,1,'i');
            }
        }
...
void parse(int idx_line_temp, int  eventual_end_line){
    ...
    while( (global_ip <= line_idx_program) || (global_ip <= eventual_end_line) ){
        ...
        else if( starts_with (program[global_ip].instruction, "vd_") ){/*dichiara funzione _ */ }
        else if( starts_with (program[global_ip].instruction, "__") ){/*chiama funzione oppure semplicemente sposta l'esecuizione ad una riga e imposta un ritorno (pipe e fork) */ }
...
void build_state() {
    ...
    while (i <= line_idx_program) {
        ...
        if (strchr(program[i].instruction, '{') != NULL) {
            ...
            else if( starts_with(program[i].instruction,"vd_") ){ 
                     char bin[64], bin_name[16], name[24]; 
                     sscanf(program[i].instruction,"vd_%15[^(](%63s",bin_name,bin);
                     sprintf(name,"vd_%s",bin_name); //SINGOLA
...
int main(int argc, char *argv[]) {
    ...
    size_t mem_kb = 500;
    char *flag_debug = NULL;
    char *file_name  = NULL;

    for (int i = 1; i < argc; i++) {
        if (strncmp(argv[i], "-kmem", 5) == 0)
            sscanf(argv[i], "-kmem%zu", &mem_kb);
        else if (strcmp(argv[i], "-df") == 0 || strcmp(argv[i], "-dt") == 0)
            flag_debug = argv[i];
        else
            file_name = argv[i];
    }
    ...
    if (flag_debug && strcmp(flag_debug, "-df") == 0) deb = fal;
    else if (flag_debug && strcmp(flag_debug, "-dt") == 0) deb = tru;
    ...
    if (file_name) {
        ...
        char *code = read_code_from_file(file_name);
        format_code(code);
        if(deb) printf("DEBUG: uscito da format_code\n\n\n");
        build_state();
        ...
        if(deb) {
            ...
            int i = 0;
            while(i <= line_idx_program){
                printf("%d, %s\n", program[i].line_number, program[i].instruction);
                i++;
            }
            printf("Stato costruito:\n");
            i = 0;
            while(i < return_state){
                printf("n%d: riga %d nome: %s, skip %d\n",
                    state_stack[i].codice, state_stack[i].posizione_ritorno,
                    state_stack[i].nome_function, state_stack[i].posizione_skip);
                i++;
            }
            printf("END STATE DEBUG:\n");
        }
    ...
    if(argc == 1){
        ...
        printf("in order: ./Zinterpreter -df -kmem500 file_name.Zinter\n");
        printf("  -df  = debug false\n");
        printf("  -dt  = debug true\n");
        printf("  -kmemN = alloca N KB (default 500)\n\n");
