...
void exec_print(char *text){
    ...
    if(n < 1){
        ...
        if(strcmp(what,"variable.i") == 0){
            ...
            printf("WARNING: per chiamare una funzione usa la sintassi __name(args)\n");
        }
...
void* exec_funarg(char *name_plus_args, int op_code){
    static int test_return_value = 99;
    int current_line = global_ip;

    /*QUI VENGONO DISTINTI I VARI TIPI DI CHIAMATA DI UNA FUNZIONE: NON RITORNA ARGOMENTI __NAME
                                                                    RITORNA UN ARGOMENTO CON VAR = __NAME
    ...
    /*VARIABILI*/
        ...
        if (right_operand[0] == '\'') { /*var x k*/
            ...
            *dest = right_operand[1];
            ...
            strcpy(bin, right_name); is_what(bin);
            sscanf(bin, "%15[^.].%c", rtdata, &rtype);
            if (strcmp(rtdata, "funct") != 0) { printf("ERROR: %s is not a function\n", right_name); return; }
...
void parse(int idx_line_temp){
    ...
    while(global_ip<=line_idx_program){
        ...
        else if( strchr (program[global_ip].instruction, '=') ){ exec_equal(program[global_ip].instruction); /*exec_math(program[global_ip].instruction); funzioni: + - * / e assegnazione */}
