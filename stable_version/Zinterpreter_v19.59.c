    ...
    if(strchr(type,'[')){   //&i[ qualcosa ]&name&
        ...
        if( sscanf(type,"%c[%15[^]]]",&type_of_type,name_of_type) == 2 && type_of_type == 'i' ){
            ...
            if(sscanf(name_of_type, "%d", &size) == 1){
                ...
                if(ptr0 == NULL){
                    printf("ERROR: segmentation error for ptr0 in exec_int on line: %s \n",text);
                    return;
                    }
    ...
    if(strchr(type,'[')){   //&i[ qualcosa dimensione ]&name&
        if( sscanf(type,"%c[%15[^]]]",&type_of_type,name_of_type) == 2 && type_of_type == 's' ){
            ...
            if(sscanf(name_of_type, "%d", &size) == 1){
                ...
                int *ptr0 = get_index(name_of_type);
                if(ptr0 == NULL){
                    printf("ERROR: segmentation error for ptr0 in exec_char on line: %s \n",text);
                    return;
                    }
...
void exec_configure_set_to(char *text) {
    ...
    if (text[0] == '=') {
        printf("ERROR: im crying operetion not allowed \n"); return;
    }
    ...
    char left_i[16], right_i[16];
    char left_j[16], right_j[16];

    /*===========INTERNAL PARSE==========*/

    /*MATRIX*/
    if(strstr(left_operand, "][") && strstr(right_operand, "][")) { /*matr x matr*/
        sscanf(left_operand,  "[%[^]]][%[^]]]%s", left_i,  left_j,  left_name);
        sscanf(right_operand, "[%[^]]][%[^]]]%s", right_i, right_j, right_name);
        printf("[CHECK] matr x matr | left: [%s][%s]%s | right: [%s][%s]%s\n", left_i, left_j, left_name, right_i, right_j, right_name);

        char bin[16];
        strcpy(bin,left_name);
        is_what(bin);
        char ltdata[16] = {0};
        char ltype;
        ...
        if(ltype == rtype){
            /*COPY VALUE WITH TYPE RTYPE*/
            void *dest = resolve(ltype,left_name);
            dest = resolve(rtype,right_name);
        }

    }

    else if(strstr(left_operand, "][") && (strstr(right_operand, "][")==NULL)) { /*matr x boh*/
        sscanf(left_operand, "[%[^]]][%[^]]]%s", left_i, left_j, left_name);

        if(right_operand[0] == '\''){ /*matr x k*/
            printf("[CHECK] matr x k | left: [%s][%s]%s | right: %s\n", left_i, left_j, left_name, right_operand);
        }

        else if(isdigit((unsigned char)right_operand[0])){ /*matr x n*/
            printf("[CHECK] matr x n | left: [%s][%s]%s | right: %s\n", left_i, left_j, left_name, right_operand);
        }

        else if(strchr(right_operand,']')){ /*matr x arr*/
            sscanf(right_operand, "[%[^]]]%s", right_i, right_name);
            printf("[CHECK] matr x arr | left: [%s][%s]%s | right: [%s]%s\n", left_i, left_j, left_name, right_i, right_name);
        }

        else{ /*matr x var*/
            sscanf(right_operand, "%s", right_name);
            printf("[CHECK] matr x var | left: [%s][%s]%s | right: %s\n", left_i, left_j, left_name, right_name);
        }

    }

    else if((strstr(left_operand, "][")==NULL) && strstr(right_operand, "][")){  /*boh x matr*/
        sscanf(right_operand, "[%[^]]][%[^]]]%s", right_i, right_j, right_name);

        if(strchr(left_operand,']')){ /*arr x matr*/
            sscanf(left_operand, "[%[^]]]%s", left_i, left_name);
            printf("[CHECK] arr x matr | left: [%s]%s | right: [%s][%s]%s\n", left_i, left_name, right_i, right_j, right_name);
        }

        else{ /*var x matr*/
            sscanf(left_operand, "%s", left_name);
            printf("[CHECK] var x matr | left: %s | right: [%s][%s]%s\n", left_name, right_i, right_j, right_name);
        }
    }
    ...
    /*ARRAY*/
        ...
        sscanf(left_operand,  "[%[^]]]%s", left_i,  left_name);
        sscanf(right_operand, "[%[^]]]%s", right_i, right_name);
        printf("[CHECK] arr x arr | left: [%s]%s | right: [%s]%s\n", left_i, left_name, right_i, right_name);
    ...
    else if(strchr(left_operand,']') && (strchr(right_operand,']')==NULL)){ /*arr x boh*/
        sscanf(left_operand, "[%[^]]]%s", left_i, left_name);

        if(right_operand[0] == '\''){ /*arr x k*/
            printf("[CHECK] arr x k | left: [%s]%s | right: %s\n", left_i, left_name, right_operand);
        }

        else if(isdigit((unsigned char)right_operand[0])){ /*arr x n*/
            printf("[CHECK] arr x n | left: [%s]%s | right: %s\n", left_i, left_name, right_operand);
        }

        else{ /*arr x var*/
            sscanf(right_operand, "%s", right_name);
            printf("[CHECK] arr x var | left: [%s]%s | right: %s\n", left_i, left_name, right_name);
        }
    }

    else if((strchr(left_operand,']')==NULL) && strchr(right_operand,']')){ /*var x arr*/
        sscanf(left_operand,  "%s",        left_name);
        sscanf(right_operand, "[%[^]]]%s", right_i, right_name);
        printf("[CHECK] var x arr | left: %s | right: [%s]%s\n", left_name, right_i, right_name);
    }

    /*VARIABILI*/
    else if((strchr(left_operand,']')==NULL) && (strchr(right_operand,']')==NULL)){

        if(right_operand[0] == '\''){ /*var x k*/
            sscanf(left_operand, "%s", left_name);
            printf("[CHECK] var x k | left: %s | right: %s\n", left_name, right_operand);
            ...
            char *src = (char *)resolve(rtype,right_operand);
            ...
            *dest = *src;
            ...
            sscanf(left_operand, "%s", left_name);
            printf("[CHECK] var x n | left: %s | right: %s\n", left_name, right_operand);
            ...
            int *src = (int *)resolve(rtype,right_operand);
            if (!dest || !src) { printf("ERROR: null pointer\n"); return; }
            *dest = *src;
            ...
            sscanf(left_operand,  "%s", left_name);
            sscanf(right_operand, "%s", right_name);
            printf("[CHECK] var x var | left: %s | right: %s\n", left_name, right_name);
...
void parse(int idx_line_temp){
    ...
    while(global_ip<=line_idx_program){
        ...
        else if( strchr (program[global_ip].instruction, '=') ){ exec_configure_set_to(program[global_ip].instruction); /*exec_math(program[global_ip].instruction); funzioni: + - * / e assegnazione */}
...
void build_state() {
    ...
    while (i <= line_idx_program) {
        ...
        if (strchr(program[i].instruction, '{') != NULL) {
            ...
            else if( starts_with(program[i].instruction,"__") ){ 
                     char bin[64], bin_name[16], name[24]; 
                     sscanf(program[i].instruction,"__%15[^(](%63s",bin_name,bin);
                     sprintf(name,"__%s",bin_name); 
