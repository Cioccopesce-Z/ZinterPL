...
int is_function_(const char *name_or_declaration){
    ...
    if(strstr(name_or_declaration,"od_")){ // od_ name(args){ } being declared 
        ...
        sscanf(name_or_declaration,"__%63[^(](%63[^)])",name,args); //name == "name"  args == "args"
    } 
...
int main(int argc, char *argv[]) {
    ...
    if(argc >= 3){
        ...
        if (!dot || (strcmp(dot, ".Zim") != 0 && strcmp(dot, ".Zinter") != 0) && strcmp(dot, ".zim") != 0) {
        ...
        if(argc >= 4){
            ...
            for(int i = 3; i<argc; i++){
                if(strstr(argv[i],".Zlib") || strstr(argv[i],".zlib")){
