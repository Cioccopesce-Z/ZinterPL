...
int check_return(int eventual_end_line) {
    if (eventual_end_line == line_idx_program) {
...
void parse(int idx_line_temp, int  eventual_end_line){
    ...
    while( !return_hit && (global_ip <= line_idx_program) ){
...
void importlib(const char *libfile, const char *codefile){
    ...
    FILE *tmp = fopen(".__temp__.Zinter","w");
    if(!tmp){
