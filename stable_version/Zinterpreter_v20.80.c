...
void exec_math(char *text){
    ...
    if(starts_with(text,"=&")){
        ...
        if(check_if_same_type(type1,type2)){
            ...
            printf("ERROR: different type on = in: %s\n",text);
            return;
        }
    }
...
void exec_set_to(char *text){
    ...
    if(strcmp(_set_to_what,"variable") == 0){
        ...
        if(type == 'l'){
            ...
            int *nindex= (int *)get_index(index);
            if(nindex == NULL){ printf("ERROR: cant get %s index in %s\n",value,text); return;}
        ...
        if(type == 'l'){
            ...
            set_to_matrix(name,type,*nindex1,*nindex2,*nvalue,char_place_holder);
        }
            ...
            set_to_matrix(name,type,*nindex1,*nindex2,place_holder,*nvalue);
        }
    }
